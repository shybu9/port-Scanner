...
argparse = argparse.ArgumentParser(description=f"{pink} THIS TOOL IS USED TO SCAN THE PORTS IN A NETWORK\n{reset}",
                                   usage=f"{blue}python3 {sys.argv[0]}{pink} -i {reset}<target ip_address>"
                                         ...
                                         f"{pink} -v {reset}<verbose level ( 0 for verbose {cyan}OFF{reset} and  1 for verbose {cyan}ON {reset})>"
...
argparse.add_argument("-V", "--verbose", help=f"{green}use 0 for no verbose and 1 for verbose output{reset}", required=True)
